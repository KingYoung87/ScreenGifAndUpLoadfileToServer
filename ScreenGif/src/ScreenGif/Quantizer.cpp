	...
	NODE** pReducibleNodes)
{
	...
	for (int j = 0; j<8; j++)
	{
		if (pNode->pChild[j] != NULL)
		{
			nRedSum += pNode->pChild[j]->nRedSum;
			nGreenSum += pNode->pChild[j]->nGreenSum;
			nBlueSum += pNode->pChild[j]->nBlueSum;
			pNode->nPixelCount += pNode->pChild[j]->nPixelCount;
			free(pNode->pChild[j]);
			pNode->pChild[j] = NULL;
			nChildren++;
		}
